...
//         n = recvfrom(server_socket, buffer, 100, 0, (struct sockaddr *)&server_addr, &addr_size);
//         if (n < 0)
//         {
...
void send_data(char data[], int server_socket, socklen_t addr_size, struct sockaddr_in client_addr)
{
    ...
    if (activity > 0 && FD_ISSET(server_socket, &read_fds))
    {
        ...
        if (n > 0)
        {
            ...
            {
                printf("Server received all packets successfully.\n");
            }
        }
    }
...
int main()
{
    ...
    char buff[] = "I'm connected";
    sendto(server_socket, buff, 100, 0, (struct sockaddr *)&server_addr, addr_size);
    ...
    while (1)
    {
        ...
        send_data(data, server_socket, addr_size, server_addr);
